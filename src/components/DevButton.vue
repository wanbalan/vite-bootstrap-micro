<template>
  <div class="row">
    <div class="col">
      <h1>
        <strong class="text-muted">{{ deviceSetting().col1Title }}</strong>
      </h1>
      <vibe-icon
        @click="turn_one_output('CH1_CH2', 'generator-one')"
        type="button"
        icon="plugin"
        class="fs-2 text-primary p-1"
      ></vibe-icon>
      <vibe-button-group size="sm" class="me-2 pb-2">
        <vibe-button
          @click="turn_one_output('CH1', 'generator-one')"
          variant="outline-secondary"
          :class="store.g1_output1 ? 'text-primary' : ''"
          >CH1</vibe-button
        >
        <vibe-button
          @click="turn_one_output('CH2', 'generator-one')"
          variant="outline-secondary"
      ...
      <div v-show="store.list_one_collumn" class="col">
        <vibe-icon
          @click="turn_one_output('CH1_CH2', 'generator-two')"
          v-show="store.list_one_collumn"
          type="button"
          icon="plugin"
          class="fs-2 text-success p-1"
        ></vibe-icon>
        <vibe-button-group size="sm" class="me-2 pb-2">
          <vibe-button
      ...
      <div
        class="container p-1 justify-content-center"
    ...
    <div class="col" v-show="!store.list_one_collumn">
      <h1>
        <strong class="text-muted">{{ deviceSetting().col2Title }}</strong>
      </h1>
      <vibe-icon
        @click="turn_one_output('CH1_CH2', 'generator-two')"
        type="button"
        :class="
          with_one_gen.includes(store.title) ? 'text-primary' : 'text-success'
        "
        icon="plugin"
        class="fs-2 p-1"
      ></vibe-icon>
...
function text(but) {
  if (store.title == "ЭСУ-436" || store.title == "ЭСУ-222") {
    ...
    return Object.keys(but).length === 1
      ? but.gen1.ch1.gz + " Гц <br>" + but.gen1.ch2.gz + " Гц "
      : but.gen2.ch1.gz + " Гц <br>" + but.gen2.ch1.U + " mV";
...
function clickOnButtonFromOneColumn(index, but) {
  ...
  if (store.title == "ЭСУ-436" || store.title == "ЭРРД-436") {
    store.sendPostRequest(
      `SOUR1:APPL:SQU ${but.gen1.ch1.gz},${but.gen1.ch1.U};:SOUR2:APPL:SQU ${but.gen1.ch2.gz},${but.gen1.ch2.U}\r\n`,
      "generator-one",
    );
  } else if (store.title == "ЭСУ-222") {
    store.sendPostRequest(
      `SOUR1:APPL:SIN ${but.gen1.ch1.gz},${but.gen1.ch1.U};:SOUR2:APPL:SIN ${but.gen1.ch2.gz},${but.gen1.ch2.U}\r\n`,
      "generator-one",
    );
  } else if (store.title == "ЭРРД-1700") {
    store.sendPostRequest(
    ...
    store.sendPostRequest(
      `SOUR1:APPL:SIN ${but.gen2.ch1.gz},${but.gen2.ch1.U}mvrms;:SOUR2:APPL:SIN ${but.gen2.ch2.gz},${but.gen2.ch2.U}mvrms\r\n`,
      "generator-two",
    );
